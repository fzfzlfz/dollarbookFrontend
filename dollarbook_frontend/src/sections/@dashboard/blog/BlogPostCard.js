...
export default function BlogPostCard(props) {
  ...
  const photos = photoslink.map((link,index) => 
  <Avatar key={index} src={link} sx={{ width: 100, height: 100 }} onClick={() => props.clickPic(link, index)} onMouseOver={() => props.onChosePic(link)}/>
  );
